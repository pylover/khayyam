...
from jalali_date import JalaliDate, MINYEAR, MAXYEAR, _replace_if_match
...
class JalaliDatetime(JalaliDate):
    ...
    def from_datetime(cls, dt, tz=None):
        ...
        arr = jalali_date_from_julian_days(julian_days)
        if not tz:
            tz = dt.tzinfo
...
"""     
        ...
        result = super(JalaliDatetime, self).strftime(frmt) 
        
        result = _replace_if_match(result, '%H', self.hour)
        
        result = _replace_if_match(result, '%I', self.hour12)
        
        result = _replace_if_match(result, '%M', self.minute)
        
        result = _replace_if_match(result, '%S', self.second)
        
        result = _replace_if_match(result, '%f', self.microsecond)
        
        result = _replace_if_match(result, '%c', self.localshortformat)
        result = _replace_if_match(result, '%C', self.localformat)
        
        result = _replace_if_match(result, '%p', self.ampm)
        
        result = _replace_if_match(result, '%X', self.localtimeformat)
        
        result = _replace_if_match(result, '%z', self.utcoffsetformat)
        
        result = _replace_if_match(result, '%Z', self.tznameformat)
